...
class RankingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            images_list = []
            for file_names in self.table.column('image_path')[idx].as_py():
                img_path = os.path.join(self.image_folder, file_names)
                images = read_image(img_path, mode=ImageReadMode.RGB).float()
                images = self.transforms(images)
                images_list.append(images)
            label = self.table.column('rank')[idx].as_py()
            caption = self.tokenizer(self.table.column('prompt')[idx].as_py())
            return images_list, label, caption
        except:
            return self.__getitem__((idx + 1) % len(self))
...
def collate_rank(batch):
    images = [torch.stack(sample[0]) for sample in batch]
    images = [g[:4] if g.size(0) > 4 else g for g in images]
    labels = [torch.tensor(sample[1][:4]) if len(sample[1]) > 4 else torch.tensor(sample[1]) for sample in batch]
    captions = [sample[2][:4] if len(sample[2]) > 4 else sample[2] for sample in batch]
    num_images = torch.tensor([g.size(0) for g in images])
    # labels = [torch.tensor(sample[1]) for sample in batch]
    # captions = torch.cat([sample[2] for sample in batch])
    images = torch.cat(images)
    labels = torch.cat(labels)
    captions = torch.cat(captions) # MIGHT NOW WORK
    return images, num_images, labels, captions
