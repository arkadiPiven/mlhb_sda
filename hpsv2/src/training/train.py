...
class SDAAggregator(torch.nn.Module):
    def __init__(self, clip_model, embed_dim=1024, hidden_dim=256, freeze_clip=True):
        super().__init__()
        self.clip_model = clip_model
        self.freeze_clip = freeze_clip

        # Example aggregator networks
        ...
        self.set_transform = nn.Sequential(
    ...
    def forward(self, images, texts, cast_dtype=None):
        """
        images: shape (batch_size, total_images, 3, H, W)
        texts: shape (batch_size, tokens) or None if no text
        returns: (batch_size, total_images) of set-dependent scores
        """
        B, N, C, H, W = images.shape

        # --- 1) Encode text (if you have text prompts) ---
        if texts is not None:
            with torch.no_grad() if self.freeze_clip else torch.enable_grad():
                text_emb = self.clip_model.encode_text(texts)  # (B, embed_dim)
        else:
            text_emb = None
        ...
        with torch.no_grad() if self.freeze_clip else torch.enable_grad():
            img_emb_flat = self.clip_model.encode_image(images_flat)  # (B*N, embed_dim)

        img_embs = img_emb_flat.view(B, N, -1)  # => (B, N, embed_dim)

        # --- 3) Aggregation (DeepSets) ---
        item_feats = self.item_transform(img_embs)  # (B, N, hidden_dim)
        pooled = item_feats.sum(dim=1)  # (B, hidden_dim)

        if text_emb is not None:
            prompt_feats = self.prompt_transform(text_emb)  # (B, hidden_dim)
        else:
            prompt_feats = torch.zeros_like(pooled)

        combined = torch.cat([pooled, prompt_feats], dim=1)
        set_emb = self.set_transform(combined)  # (B, hidden_dim)

        set_emb_expanded = set_emb.unsqueeze(1).expand(-1, N, -1)  # (B, N, hidden_dim)
        merged = torch.cat([item_feats, set_emb_expanded], dim=-1)  # (B, N, hidden_dim*2)
        scores = self.score_head(merged).squeeze(-1)  # (B, N)
